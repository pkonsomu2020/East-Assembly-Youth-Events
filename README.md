...
- **Youth Retreat / Chill Out dates** — currently marked "date to be communicated" (`src/data/events.ts`); once you have dates, I can add them to the event cards and calendar the same way Dinner and Worship Experience are set up.
- **Books** — the merchandise page has a placeholder "coming soon" card; send me the price and details whenever you have them.
- **Event calendar months** — the calendar on the Events page currently only renders August and October 2026 (`src/components/events/EventCalendar.tsx`), matching the two events with confirmed dates. It can be made to auto-generate from the events list once more dates are confirmed.
